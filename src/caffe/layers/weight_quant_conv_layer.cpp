...
void WeightQuantConvolutionLayer<Dtype>::LayerSetUp(
  ...
  handles_setup_ = true;
	...
	else
	{
		if(this->blobs_.size() == 1)
		{
			...
			// for safety, make enough space for quantize_base
...
template <typename Dtype>
void WeightQuantConvolutionLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
WeightQuantConvolutionLayer<Dtype>::~WeightQuantConvolutionLayer() {
...
INSTANTIATE_CLASS(WeightQuantConvolutionLayer);
REGISTER_LAYER_CLASS(WeightQuantConvolution);
